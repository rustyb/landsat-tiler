...
@rr_cache()
@LANDSAT_APP.route('/landsat/s2/bounds/<scene>', methods=['GET'], cors=True)
def s2_bounds(scene):
    """
    Handle bounds requests
    """
...
@rr_cache()
@LANDSAT_APP.route('/landsat/s2/metadata/<scene>', methods=['GET'], cors=True)
def s2_metadata(scene):
    """
    Handle metadata requests
    """
    # query_args = LANDSAT_APP.current_request.query_params
    ...
    info = sentinel2.metadata(scene)
    return ('OK', 'application/json', json.dumps(info))

@rr_cache()
@LANDSAT_APP.route('/landsat/s2/tiles/<scene>/<int:z>/<int:x>/<int:y>.<ext>', methods=['GET'], cors=True)
def s2_tile(scene, tile_z, tile_x, tile_y, tileformat):
    """
    Handle tile requests
    """
    query_args = LANDSAT_APP.current_request.query_params
    ...
    rgb = map(str, rgb.split(',')) if isinstance(rgb, str) else rgb
    rgb = tuple(rgb)
